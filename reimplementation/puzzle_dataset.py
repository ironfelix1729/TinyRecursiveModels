...
class PuzzleDataset(IterableDataset):
    def __init__(self, config: PuzzleDatasetConfig, split: str = "train"):
        ...
        mean_puzzle_examples = mean_puzzle_examples / total_puzzles if total_puzzles > 0 else 0
        self.metadata = PuzzleDatasetMetadata(
            seq_len=prev_seq_len, vocab_size=prev_vocab_size, pad_id=prev_pad_id, ignore_label_id=prev_ignore_label_id,
            blank_identifier_id=prev_blank_identifier_id, num_puzzle_identifiers=num_identifiers,
            total_groups=total_groups, mean_puzzle_examples=mean_puzzle_examples, total_puzzles=total_puzzles, sets=prev_sets
        )
    ...
    def _iter_train(self):
        for set_name, dataset in self._data.items():
            ...
            while start_index < group_order.size:
                start_index, batch_indices, batch_puzzle_indices = _sample_batch(rng, group_order, dataset["puzzle_indices"], dataset["group_indices"], start_index, self.config.global_batch_size)
                global_effective_batch_size = batch_puzzle_indices.size
                if global_effective_batch_size < self.config.global_batch_size: break
